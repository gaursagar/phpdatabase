<?php
    ...
    if (isset($_SESSION['Admin'])) {
        if(isset($_POST["submit"])) {
          ...
          $total_stock = stripslashes($total_stock);   //Make Sure Date Format is Correct
          $description = stripslashes($description);

          //Disallow Null Values to be inserted
          if ($name != "" && $total_stock != "" && $total_stock != "" && $manufacturer != "" && $distance !="") {
              $query = "INSERT INTO `vintagebikes`.`spareproposed` 
                (`spare_id`, `name`, `total_stock`, `description`, `cost`) 
                VALUES (NULL, '$name', '$total_stock', '$description', '$cost';";
              // echo $query;
              $result = mysql_query($query);
              mysql_close();
              if ($result == 1) {
                header("location:viewadminmanufacturer.php");
              }
              else{
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-6 centered">
                        <form action = "adminmanufacturer.php" class="well" method = "POST">
